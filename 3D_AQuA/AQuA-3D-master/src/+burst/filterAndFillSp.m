...
[H,W,TH,T] = size(lblMap);
lblMap2F = zeros(H,W,TH,T,'uint32');
...
for ii=1:min(nLm,numel(pixLstCut))
    ...
    %res00 = resCell{ii};
    if ~isempty(pix00)
        [ih,iw,il,~] = ind2sub([H,W,TH,T],pix00);
        fiux0g = unique(sub2ind([H,W,TH],ih,iw,il));
        if numel(fiux0g)>9 || numel(ih)>27
...
for tt=1:T
    ...
    for ii=1:Lhcc.NumObjects
        ...
        [ih,iw,il] = ind2sub([H,W,TH],pix0);
        neibLst = [];
        isGood = 1;
        for jj=1:numel(dh)
            ih1 = ih+dh(jj); ih1 = max(min(ih1,H),1);
            iw1 = iw+dw(jj); iw1 = max(min(iw1,W),1);
            il1 = il+dl(jj); il1 = max(min(il1,TH),1);
            pix1 = sub2ind([H,W,TH],ih1,iw1,il1);
            x = L(pix1);
            x = x(x>0);
            x = unique(x);
