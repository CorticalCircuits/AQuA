...
% dFmax = max(movmean(dFip,5,4),[],4);
% dFmax(dFmax<0) = 0;
% dFInfo = medfilt3(dFmax);  % information in each pixel, for peak SNR
% dFInfo(isnan(dFInfo)) = 0;

% rising time as feature
thrMax = ceil(quantile(dFip(:),0.999)/s00);
thrVec = 0:thrMax;
m0Msk = zeros(H,W,L,T); m0Msk(seMap==seSel)=1; m0Msk(seMap>0 & seMap~=seSel) = -1;
tMapMT = burst.getSuperEventRisingMapMultiThr(dFip,m0Msk,thrVec,s00);
tMapMTValid = sum(~isnan(tMapMT),4)>0;
...
% graph
a = find(validMap>0);
[ih0,iw0,il0] = ind2sub(size(validMap),a);
gapSeed = max(ceil(max([max(ih0)-min(ih0),max(iw0)-min(iw0),max(il0)-min(il0)])/opts.gtwGapSeedRatio),opts.gtwGapSeedMin);
...
for ii=1:numel(spLst)
    [ih0,iw0,il0] = ind2sub([H,W,L],spSeedVec(ii));
    pathCell{ih0,iw0,il0} = path0{ii};
end
datWarp = gtw.warpRef2Tst(pathCell,refBase/max(refBase(:)),vMap1,[H,W,L,numel(refBase)]);
