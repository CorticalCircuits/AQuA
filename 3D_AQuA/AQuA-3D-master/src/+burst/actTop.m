function [dat,dF,arLst,lmLoc,opts,dActVox] = actTop(datOrg,opts,evtSpatialMask,ff)
    ...
    datOrgMean = mean(datOrg,4);
    msk000 = var(datOrg,0,4)>1e-8;
    ...
    stdMap = sqrt(median(xx,4)/0.9133);
    stdMapGauBef = double(imgaussfilt3(stdMap,0.5));
    ...
    if opts.smoXY>0
        for tt=1:size(dat,4)
            dat(:,:,:,tt) = imgaussfilt3(dat(:,:,:,tt),opts.smoXY);
        end
    end
    ...
    stdMap = sqrt(median(xx,4)/0.9133);
    stdMapGau = double(imgaussfilt3(stdMap,0.5));
