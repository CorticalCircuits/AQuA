...
for nn=1:numel(dIx)
    ...
    % extract region
    [ihSeed,iwSeed,ilSeed] = ind2sub([H,W,L],iSeedIn);
    rgh = max(ihSeed-gaphw,1):min(ihSeed+gaphw,H);
    rgw = max(iwSeed-gaphw,1):min(iwSeed+gaphw,W);
    rgl = max(ilSeed-gaphw,1):min(ilSeed+gaphw,L);
    H1 = numel(rgh);
    W1 = numel(rgw);
    L1 = numel(rgl);
    ihSeed = ihSeed-min(rgh)+1;
    iwSeed = iwSeed-min(rgw)+1;
    ilSeed = ilSeed-min(rgl)+1;
    iSeed = sub2ind([H1,W1,L1],ihSeed,iwSeed,ilSeed);
    
    df0 = dF(rgh,rgw,rgl,:);
    df0Vec = reshape(df0,[],T);
    
    ft = tMapMT(rgh,rgw,rgl,:);
    availMap0 = availMapx(rgh,rgw,rgl);
    availMap0(ihSeed,iwSeed,ilSeed) = false;
    
    % distances: delay, intensity and distance
    ftBase = reshape(ft(ihSeed,iwSeed,ilSeed,:),1,[]);
    if sum(~isnan(ftBase))==0
        continue
    end
    ftVec = reshape(ft,[],size(ft,4));
    distDelay = nanmedian(abs(ftVec - ftBase),2);
    distDelay(isnan(distDelay)) = Inf;
    distInt = abs(sum(isnan(ftVec),2)-sum(isnan(ftBase)));
    [ih1,iw1,il1] = find(ones(H1,W1,L1));
    distEuc = sqrt((ihSeed-ih1).^2+(iwSeed-iw1).^2+(ilSeed-il1).^2);
    distx = distDelay+distInt*0.5+distEuc*2;
    distx = reshape(distx,H1,W1,L1);
    ...
    for ii=1:100
        ...
        [ih2,iw2,il2] = ind2sub([H1,W1,L1],newPix);
        h00 = [min(ih2+1,H1),max(ih2-1,1),ih2,ih2,ih2,ih2];
        w00 = [iw2,iw2,max(iw2-1,1),min(iw2+1,W1),iw2,iw2];
        l00 = [il2,il2,il2,il2,min(il2+1,L1),max(il2-1,1)];
        newCand = unique(sub2ind([H1,W1,L1],h00,w00,l00));
    ...
    availMapx(rgh,rgw,rgl) = availMapx(rgh,rgw,rgl).*availMap0;
    [ihx,iwx,ilx] = ind2sub([H1,W1,L1],pixSet);
    spLst0 = sub2ind([H,W,L],ihx+min(rgh)-1,iwx+min(rgw)-1,ilx+min(rgl)-1);
    dIx(mapx(spLst0)) = 0;
...
for kk=1:100
    ...
    for ii=1:numel(idx)
        idx0 = idx(ii);
        [ih,iw,il] = ind2sub([H,W,L],idx0);
        rgh = max(ih-1,1):min(ih+1,H);
        rgw = max(iw-1,1):min(iw+1,W);
        rgw = max(il-1,1):min(il+1,L);
        ix = spMap(rgh,rgw,rgl);
        ix = ix(ix>0);
        if ~isempty(ix)
            spMap(idx0) = ix(1);
        end
