...
% seed loc
[ihSeed,iwSeed,ilSeed,~] = ind2sub(res.MovSz,res.iSeed);
ihSeed = ihSeed-min(res.rgH)+1;
iwSeed = iwSeed-min(res.rgW)+1;
ilSeed = ilSeed-min(res.rgL)+1;
ihwSeed = sub2ind([H,W,L],ihSeed,iwSeed,ilSeed);
...
% init
fiux = zeros(H,W,L);  % included pixels
pixBad = zeros(H,W,L);  % weak pixels and excluded pixels
twMap = zeros(H*W*L,5);  % time window for each pixel: start, stop, 50% rise, 50% fall, peak time
if res.stg==1
    fiux(res.fiux) = 1;
    pixBad(res.pixBad) = 1;
    twMap(res.fiux,:) = res.twMap-tx0+1;
else
    fiux(ihSeed,iwSeed,ilSeed) = 1;
    fiux(pixBad>0) = 0;
end
