function [ftsLst,dffMat,dMat] = getFeaturesTop(dat,evtLst,opts)
    ...
    for ii=1:numel(evtLst)
        ...
        % dff
        voxd1 = dat(rgH,rgW,rgL,:);
        voxd1 = reshape(voxd1,[],T);
        voxi1 = evtMap(rgH,rgW,rgL,:);
        ...
        voxd2 = reshape(datx(rgH,rgW,rgL,:),[],T);
        idx = sub2ind([numel(rgH),numel(rgW),numel(rgL),T],ih-min(rgH)+1,iw-min(rgW)+1,il-min(rgL)+1,it);
        ...
        ftsLst.loc.x4D{ii} = pix0;
        ftsLst.loc.x3D{ii} = ihw;
        ...
        il1 = il-min(rgL)+1;
        it1 = it-min(rgT)+1;
        voxd = dat(rgH,rgW,rgL,rgT);
        voxi = zeros(size(voxd));
        pix1 = sub2ind(size(voxd),ih1,iw1,il1,it1);
        voxi(pix1) = 1;
