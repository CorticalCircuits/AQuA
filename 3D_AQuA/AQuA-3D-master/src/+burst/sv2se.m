...
while 1
    ...
    [ih,iw,il,it] = ind2sub([H,W,L,T],msk0);
    rgh = min(ih):max(ih);
    rgw = min(iw):max(iw);
    rgl = min(il):max(il);
    rgt = min(it):max(it);
    ...
    L1 = numel(rgl);
    T1 = numel(rgt);
    msk0 = sub2ind([H1,W1,L1,T1],ih-min(rgh)+1,iw-min(rgw)+1,il-min(rgl)+1,it-min(rgt)+1);
    mapS0 = zeros(H1,W1,L1,T1);
    lblMapS0 = lblMapS(rgh,rgw,rgl,rgt);
    ...
    for ii=1:numel(svVec)
        [ih0,iw0,il0,it0] = ind2sub([H,W,L,T],spVoxLst{svVec(ii)});
        ih0 = ih0-min(rgh)+1;
        iw0 = iw0-min(rgw)+1;
        il0 = il0-min(rgl)+1;
        it0 = it0-min(rgt)+1;
        ...
        il0 = min(max(il0,1),L1);
        it0 = min(max(it0,1),T1);
        vox00 = sub2ind([H1,W1,L1,T1],ih0,iw0,il0,it0);
        mapS0(vox00) = 0;
    end
    ...
    if ccL1.NumObjects>0
        ...
        for ii=1:numel(ccLst1)
            [ih0,iw0,il0,it0] = ind2sub([H1,W1,L1,T1],ccLst1{ii});
            ih0 = ih0+min(rgh)-1;
            iw0 = iw0+min(rgw)-1;
            il0 = il0+min(rgl)-1;
            it0 = it0+min(rgt)-1;
            ccLst1{ii} = sub2ind([H,W,L,T],ih0,iw0,il0,it0);
        end
